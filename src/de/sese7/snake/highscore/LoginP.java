...
public class LoginP extends JPanel{
		...
		if(dbversion.equalsIgnoreCase(Main.version)){
			...
			falselogin.addMouseListener(new MouseAdapter() {
				   ...
				   public void mouseClicked(MouseEvent e) {
				    	...
				    	{
				    	   try {
								Desktop.getDesktop().browse(new URI("http://www.sese7.de/#snake"));
	...
	private void registerGraphics(){
		...
		loginlabel = new JLabel("<html><div style=\"text-align: center;\">Sese7.de<br />Highscore Userlogin</div></html>");
		...
		register.addMouseListener(new MouseAdapter() {
			   ...
			   public void mouseClicked(MouseEvent e) {
			    	if(Desktop.isDesktopSupported())
			    	{
			    	   try {
			    		   Spielfeld.pause();
			    		   Desktop.getDesktop().browse(new URI("http://www.sese7.de/#register"));
						} catch (IOException | URISyntaxException e1) {
							e1.printStackTrace();
						}
			    	  }
